...
En el ejemplo anterior, la caja más pequeña entra en la caja más grande. Por lo tanto, 
es posible empaquetar todas las cajas en una sola. Ahora veamos un caso que no:
...
En el ejemplo anterior, la primer caja cabe en la tercera, y la tercera en la segunda. 
Por lo tanto, es posible empaquetar todas las cajas en una sola.
...
function fitsInOneBox(unsortedBoxes) {
    
    let sortedBoxes = unsortedBoxes.sort((actualBox, nextBox) => {
        if((actualBox.l > nextBox.l) || (actualBox.w > nextBox.w) || (actualBox.h > nextBox.h))
            return 1;
    
        return -1;
    });

    return sortedBoxes.every((actualBox, boxIndex) => {
    
        if((boxIndex + 1) < boxes.length)
            return (actualBox.l < boxes[boxIndex + 1].l) && (actualBox.w < boxes[boxIndex + 1].w) && (actualBox.h < boxes[boxIndex + 1].h);
        else return true;
