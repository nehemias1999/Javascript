...
Cada reno tiene un límite de peso máximo que puede llevar. El límite de peso máximo de 
cada reno es igual a dos veces el número de letras en su nombre.
...
de cajas de estos regalos que Santa Claus puede entregar a los niños. 
...
function distributeGifts(packOfGifts, reindeers) {
    let totalReindeersWeight = reindeers.reduce((totalWeight, reindeerWeight) => {
        return totalWeight + (reindeerWeight.length * 2);
    }, 0);

    let totalGiftsWeight = packOfGifts.reduce((totalWeight, giftWeight) => {
        return totalWeight + giftWeight.length;
    }, 0);

    return Math.trunc(totalReindeersWeight / totalGiftsWeight);
}
