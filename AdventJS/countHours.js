...
están preparando un programa que les diga el número de horas extras que harían en el 
año si se aplicara la nueva norma.
...
function countHours(year, holidays) {
    let amountOfExtraHours = 0;

    holidays.forEach(holidayDate => {
        let [month, day] = holidayDate.split("/");
        let dateToExamine = new Date(year, month - 1, day);

        let dayInTheWeek = dateToExamine.getDay();
        if((1 <= dayInTheWeek) && (dayInTheWeek <= 5)) 
            amountOfExtraHours += 2;
    });

    return amountOfExtraHours;
}
